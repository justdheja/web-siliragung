...
const ContentCard = ({ content }) => {
  const { title, thumbnail, slug } = content.fields;
	...
	return (
		<div className="overflow-hidden shadow-xl mt-4 h-90 w-60 cursor-pointer m-auto inline-block">
				...
				<div className="bg-white w-full p-4">
					...
					<div className="flex items-center">
						<div className="flex flex-col justify-between text-sm">
							...
							<p className="text-gray-400 ">{new Date(content.sys.createdAt).toDateString()}</p>
						</div>
					</div>
				</div>
			</a>
